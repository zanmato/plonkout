...
import type { Id, Workout, WorkoutTemplate } from "@/types/domain";
...
async function loadTemplates() {
  try {
    ...
    templates.value = data.sort(
      // Missing dates sort as NaN, same as subtracting two invalid dates
      (a, b) =>
        new Date(b.created ?? NaN).getTime() -
        new Date(a.created ?? NaN).getTime()
    );
  } catch (error) {
 ...
 * Delete a template
 */
async function deleteTemplate(templateId: Id) {
  try {
 ...
 */
function getTemplateSummary(template: WorkoutTemplate): string {
