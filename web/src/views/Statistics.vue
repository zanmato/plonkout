...
const workoutsChartCanvas =
  useTemplateRef<HTMLCanvasElement>("workoutsChartCanvas");
const durationChartCanvas =
  useTemplateRef<HTMLCanvasElement>("durationChartCanvas");
const volumeChartCanvas = useTemplateRef<HTMLCanvasElement>("volumeChartCanvas");

const isDark = document.documentElement.classList.contains("dark");
const textColor = isDark ? "#ffffff" : "#000000";

let workoutsChart: Chart<"bar", number[], string> | null = null;
let durationChart: Chart<"line", number[], string> | null = null;
let volumeChart: Chart<"doughnut", number[], string> | null = null;

/**
 * True for workouts with both a start and an end time
 */
function hasDuration(workout: Workout): workout is EndedWorkout {
  return Boolean(workout.started && workout.ended);
}

/**
 * Minutes between start and end of a workout
 */
 ...
 */
const topExercises = computed<ExerciseUsage[]>(() => {
  const exerciseCounts: Record<string, ExerciseUsage> = {};

  workouts.value.forEach((workout) => {
    if (workout.exercises) {
      workout.exercises.forEach((exercise) => {
        let usage = exerciseCounts[exercise.name];
        if (!usage) {
          usage = {
            name: exercise.name,
            muscleGroup:
          ...
          exerciseCounts[exercise.name] = usage;
        }
        usage.count++;
      });
    }
...
function createVolumeChart() {
  ...
  workouts.value.forEach((workout) => {
    if (workout.exercises) {
      workout.exercises.forEach((exercise) => {
        let volume = exerciseVolume[exercise.name] || 0;
        ...
        exerciseVolume[exercise.name] = volume;
      });
    }
  });
