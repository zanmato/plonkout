...
import type { Arm, Exercise, Workout } from "@/types/domain";

type BlockDisplayData = Awaited<ReturnType<typeof getBlockDisplayData>>;
type StartBlockSettings = NonNullable<Parameters<typeof startNewBlock>[2]>;
...
const emit = defineEmits<{
  (e: "close"): void;
  (e: "weightApplied", weight: number): void;
  (e: "blockStarted", exerciseName: string): void;
}>();
...
const blockData = ref<Partial<BlockDisplayData>>({});
const estimated1RM = ref(0);

// Global settings
const globalSettings = ref<BlockSettings>({
  weeksPerBlock: 5,
  workoutsPerWeek: 1,
  startPercentage: 75,
...
const hasExerciseOverride = (setting: keyof BlockSettings) => {
...
const handleStartNewBlock = async () => {
  ...
  const settings: StartBlockSettings = {};
...
const applyWeight = () => {
  if (props.onApplyWeight && (blockData.value.recommendedWeight ?? 0) > 0) {
