...
const recentExercises = ref<WorkoutExercise[]>([]);
const loading = ref(true);
const searchQuery = ref("");
const showNewExerciseForm = ref(false);

const newExercise = ref<NewExerciseForm>({
  name: "",
  muscleGroup: "",
  singleArm: true,
  type: "strength",
...
const muscleGroupOptions = computed<{ value: MuscleGroup; label: string }[]>(() => [
...
const groupedExercises = computed<ExerciseGroup[]>(() => {
  ...
  if (recentExercises.value.length > 0 && props.workoutName) {
    const recentGroup: ExerciseGroup = {
      muscleGroup: "Recent",
      exercises: [],
    };
