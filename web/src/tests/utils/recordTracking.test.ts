 ...
 */
function isArmCompatible(currentArm: Arm, historicalArm: Arm) {
 ...
 */
function getPreviousReps(exerciseName: string, weight: number | null, arm: Arm, allWorkouts: Workout[]) {
  if (!weight || weight <= 0) {
    return null;
  }
 ...
 */
function isRepRecord(exerciseName: string, weight: number | null, reps: number | null, arm: Arm, allWorkouts: Workout[]) {
 ...
 */
function isWeightRecord(exerciseName: string, weight: number | null, arm: Arm, allWorkouts: Workout[]) {
  if (!weight || weight <= 0) {
    return false;
  }
...
describe('Record Tracking', () => {
  ...
  describe('Integration scenarios', () => {
    ...
    it('should handle exercises with only warmup sets', () => {
      const workoutsWithOnlyWarmups = partial<Workout[]>([
        {
          exercises: [
            {
              name: 'Light Exercise',
