...
import type { MonthGroup, SortableWorkout } from '../helpers/fixtures'
import { ref, computed, nextTick, type Ref } from 'vue'

/** A virtual scroller row, either a month header or a workout */
interface FlattenedItem extends Partial<SortableWorkout> {
  type: 'header' | 'workout'
  monthYear?: string
  workoutCount?: number
}
...
describe('WorkoutLog Integration - Sorting Behavior', () => {
  let workouts: Ref<SortableWorkout[]>
  let mockDateFormatter: Mock<(date: Date, format: string) => string>

  beforeEach(() => {
    workouts = ref([])
    
    // Mock the date formatter (d function from i18n)
    mockDateFormatter = vi.fn((date: Date, format: string) => {
      if (format === 'month') {
  ...
  it('reproduces the exact WorkoutLog grouping logic', async () => {
    ...
    const loadWorkouts = async () => {
      ...
      workouts.value = data.sort((a, b) => new Date(b.started).getTime() - new Date(a.started).getTime())
    }
  ...
  it('verifies sorting works with various date formats and timezones', () => {
    ...
    // Apply sorting
    const sorted = testWorkouts.sort((a, b) => new Date(b.started).getTime() - new Date(a.started).getTime())
