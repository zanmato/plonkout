...
import type { MonthGroup } from '../helpers/fixtures'

/** Same as a month group, keyed by year and month index instead of a label */
type MonthKeyGroup = Omit<MonthGroup, 'monthYear'> & { monthKey: string }
...
describe('Workout Sorting Logic', () => {
  ...
  it('groups workouts by month and sorts within groups by date descending', () => {
    ...
    const sortedWorkouts = workouts.sort((a, b) => new Date(b.started).getTime() - new Date(a.started).getTime())

    // Then group by month (simplified version of groupedWorkouts logic)
    const groups: Record<string, MonthKeyGroup> = {}
  ...
  it('verifies date comparison logic works correctly', () => {
    ...
    expect(date2.getTime() - date1.getTime()).toBeGreaterThan(0) // 20th - 15th = positive
    expect(date1.getTime() - date3.getTime()).toBeGreaterThan(0) // 15th - 10th = positive
    expect(date2.getTime() - date3.getTime()).toBeGreaterThan(0) // 20th - 10th = positive

    // Test actual sort result
    const dates = [date1, date2, date3]
    const sorted = dates.sort((a, b) => b.getTime() - a.getTime()) // Descending
