...
const el = ref<PopoverMethods | null>(null);
defineExpose({
    toggle: (event: Event, target?: unknown) => el.value!.toggle(event, target),
    show: (event: Event, target?: unknown) => el.value!.show(event, target),
    hide: () => el.value?.hide()
});
</script>
